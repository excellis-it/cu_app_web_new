...
const SidebarPanel = ({
  ...
  React.useEffect(() => {
    const fetchSiteDetails = async () => {
      if (globalUser?.data?.token && !siteDetailsFetched) {
        // Avoid re-fetching if already done? Or just fetch on mount
        try {
        ...
        {menuItems.map((item, index) => (
          ...
          >
            <div className="sidebar-content">
              ...
              <span
                className={`sidebar-label ${activeIndex === index ? "active" : ""}`}
              >
                {item.label}
              </span>
            </div>
