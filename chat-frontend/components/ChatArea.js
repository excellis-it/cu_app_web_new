...
const ChatArea = ({
  ...
  const getMeetingStatus = () => {
    const startTime = selected?.isGuestMeeting
      ? selected?.meetingStartTime
      : groupDataDetails?.meetingStartTime || selected?.meetingStartTime;
    const endTime = selected?.isGuestMeeting
      ? selected?.meetingEndTime
      : groupDataDetails?.meetingEndTime || selected?.meetingEndTime;

    // Use actual start/end times if available
    const actualStartTime = selected?.isGuestMeeting
      ? selected?.startedAt
      : groupDataDetails?.startedAt || selected?.startedAt;
    const actualEndTime = selected?.isGuestMeeting
      ? selected?.endedAt
      : groupDataDetails?.endedAt || selected?.endedAt;
    ...
    if (!hasStarted && !isPastScheduledStart) {
      ...
      (hasStarted && !hasEnded) ||
      (!hasStarted && isPastScheduledStart && !isPastScheduledEnd)
    ) {
  ...
  const handleCopyLink = (e) => {
    ...
    const startTime = moment(selected?.meetingStartTime).format(
      "MM/DD/YYYY hh:mm A",
    );
  ...
  return (
    ...
    >
      {!isMeetingOverlay && (
        <div className="messagedivhead">
          <div className="messagedivheadnameOnline d-flex align-items-center">
            ...
            <div className="messageheadname">
              <span className="membername">
                {selected?.isGoogleEvent && (
                  <GoogleIcon sx={{ fontSize: "18px", color: "#4285F4" }} />
                )}
                {displayName}
              </span>
          ...
          <div>
            ...
            {!selected?.isTemp &&
              (selected?.currentUsersId?.includes(
                globalUser?.data?.user?._id,
              ) ||
                selected?.currentUsers?.some(
                  (user) => (user._id || user) === globalUser?.data?.user?._id,
                )) && (
                <>
                  {!showRoom && (
                    <CallButton
            ...
            <div className="d-none d-lg-block">
              ...
              {selected?.isTemp &&
                (selected?.currentUsersId?.includes(
                  ...
                  selected?.currentUsers?.some(
                    (user) =>
                      (user._id || user) === globalUser?.data?.user?._id,
                  )) && (
                  <>
                    {!showRoom && (
                      <MeetingStatusBanner
      ...
      {!forceChatView &&
      (selected?.isTemp === true ||
        selected?.isGoogleEvent ||
        selected?.isGuestMeeting) &&
      (selected?.isGoogleEvent ||
        selected?.isGuestMeeting ||
        moment(selected?.meetingStartTime) > moment()) ? (
        <div
          style={{
            flex: 1,
            overflowY: "auto",
        ...
        >
          <div
            style={{
              ...
              margin: "0 auto",
              boxShadow:
                "0 25px 50px -12px rgba(243, 126, 32, 0.4), 0 15px 25px -10px rgba(0,0,0,0.1)",
            }}
          >
            <div
              style={{
                    ...
                    >
                      ...
                      >
                        {selected?.isGuestMeeting
                          ...
                          : groupDataDetails?.groupDescription ||
                            selected?.groupDescription ||
                            selected?.description ||
                            "No description provided for this meeting."}
                      </div>
                  ...
                  {!selected?.isGoogleEvent && !selected?.isGuestMeeting && (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          Participants (
                          {groupDataDetails?.currentUsers?.filter(
                      ...
                      >
                        {groupDataDetails?.currentUsers
                          ...
                          .map((user, index) => {
                            const userAction =
                              groupDataDetails.participantActions?.find(
                                (action) =>
                                  String(action.userId) === String(user._id),
                              );
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      >
                        {moment(
                          selected?.isGuestMeeting
                            ? selected?.meetingStartTime
                            : groupDataDetails?.meetingStartTime ||
                                selected?.meetingStartTime,
                        ).format("ddd, MMM DD")}
                      </div>
                      <div
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      >
                        {moment(
                          selected?.isGuestMeeting
                            ? selected?.meetingEndTime
                            : groupDataDetails?.meetingEndTime ||
                                selected?.meetingEndTime,
                        ).format("ddd, MMM DD")}
                      </div>
                      <div
                ...
                {rplyMsg.textFileType === "image"
                  ...
                  : rplyMsg.textFileType === "video"
                    ...
                    : rplyMsg.textFileType === "screen_recording"
                      ? `Call Recording : ${rplyMsg.fileName}`
        ...
        {!selected?.isGoogleEvent &&
          (selected?.isTemp ? (
            (forceChatView ||
              ...
              <div
                className="border-input-container"
                style={
                  isMeetingOverlay
                    ? {
