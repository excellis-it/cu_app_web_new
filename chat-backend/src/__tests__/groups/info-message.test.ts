...
describe('POST /api/groups/info-message', () => {
  let token: string;
  let groupId: string;
  ...
  beforeEach(async () => {
    ...
    const addRes = await supertest(app)
      ...
      .set('access-token', token)
      .send({ groupId, senderId: String(member?._id), senderName: String(member?.name), message: 'info', messageType: 'text' })
      .expect(200);
    messageId = addRes.body.data?._id || addRes.body.data?.message?._id || addRes.body.data?.messageId;
  });

  it('returns info for message', async () => {
    const res = await supertest(app)
      .post('/api/groups/info-message')
      .set('access-token', token)
      .send({ msgId: messageId })
      .expect(200);
    // API may return success false when data is not found; ensure response shape
