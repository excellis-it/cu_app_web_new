...
describe('POST /api/groups/deletemsg', () => {
  let token: string;
  let groupId: string;
  ...
  beforeEach(async () => {
    ...
    const addRes = await supertest(app)
      ...
      .set('access-token', token)
      .send({ groupId, senderId: String(member?._id), senderName: String(member?.name), message: 'to delete', messageType: 'text' })
      .expect(200);
