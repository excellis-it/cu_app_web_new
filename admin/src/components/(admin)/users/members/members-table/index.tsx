...
export default function MembersTable() {
  ...
  (user_type == 'admin')
  const { data, error, refresh } = useApi('/api/admin/users/all-users', {}, 'POST');
  ...
  useEffect( () => {
    if (data.data?.length > 0) {
      if(user_type == 'admin'){
        ...
        const filteredData = data.data.filter((user: { userType: string; }) => user.userType === 'Member');
  ...
  // }, []);
    ...
    const fetchUsers = async () => {
      const response = await fetch('/api/admin/users/all-users', {
        ...
        // If you need to send a body with the request, add it here
