  ...
  return (
    <div className={cn('grid @container', className)}>
      ...
      >
        ...
        >
          <input {...getInputProps()} accept="image/png, image/gif, image/jpeg" multiple={false} />
      ...
      {isEmpty(notUploadedItems) ? (
        <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-[repeat(auto-fit,_minmax(140px,_1fr))]">
          {isEmpty(uploadedItems) &&
            Array.isArray(uploadedItems) ? uploadedItems.map((file: any, index: number) => 
            { console.log('FILE 852;',file)
              return(
              <div key={index} className={cn('relative')}>
                <figure className="group relative h-40 rounded-md bg-gray-50">

                 
                  <MediaPreview name={file?.name?file.name:""} url={file?.url?file.url:uploadedItems[0]} />
                  <button
                    type="button"
            ...
            <div key={1} className={cn('relative')}>
              <figure className="group relative h-40 rounded-md bg-gray-50">
                
                <MediaPreview name={uploadedItems[0]?.name || " "} url={defaultValue || uploadedItems[0]?.url || uploadedItems || ""} />
                <button
                  type="button"
                  className="absolute right-0 top-0 rounded-full bg-gray-700 p-1.5 transition duration-300"
                >
              ...
              <MediaCaption name={uploadedItems[0]?.name || " "} size={100} />
            </div>
          </>
        ...
        {Array.isArray(notUploadedItems) ? notUploadedItems.map((file: any, index: number) => (
          <div key={index} className={cn('relative')}>
            <figure className="group relative h-40 rounded-md bg-gray-50">
...
function MediaCaption({ name, size = null }: { name: string; size?: number | null }) {
  return (
    <div className="mt-1 text-xs">
      <p className="break-words font-medium text-gray-700">{name}</p>
      {size && <p className="mt-1 font-mono">{prettyBytes(size)}</p>}
